...
class CameraPoseInKinova(Node):
    def __init__(self):
        ...
        self.get_logger().info(
            "CameraPoseInKinova node initialized"
        )
        ...
        self.kinova_base_link_frame = self.get_parameter(
            'kinova_base_link_frame'
        ).get_parameter_value().string_value
        self.tag0_anchor_frame = self.get_parameter(
            'tag0_anchor_frame'
        ).get_parameter_value().string_value
        self.camera_frame = self.get_parameter(
            'camera_frame'
        ).get_parameter_value().string_value
        self.tag_frame = self.get_parameter(
            'tag_frame'
        ).get_parameter_value().string_value
    ...
    def on_timer(self):
        ...
        mat_ct = t3.concatenate_matrices(
            ...
            t3.quaternion_matrix((r.w, r.x, r.y, r.z))
        )
        ...
        mat_kt = t3.concatenate_matrices(
            ...
            t3.quaternion_matrix((rs.w, rs.x, rs.y, rs.z))
        )
