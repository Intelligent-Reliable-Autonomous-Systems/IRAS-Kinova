...
class ReachPolicy(Node):
    ...
    def __init__(self, fail_quietly: bool = False, verbose: bool = False):
        ...
        self.create_subscription(JointTrajectoryControllerState, self.STATE_TOPIC, self.sub_callback, 10)
...
if __name__ == "__main__":
    ...
    for act in act_list:
        actions[act] = [a for a in act_list.get(act).get("joint_names")]

    joint_names[actuator] = (
        np.array([[jn for jn in self.expand_fnmatch(j)] for j in joint_names_expr]).flatten().tolist()
    )
